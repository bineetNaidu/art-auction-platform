...
import { Eye, EyeOff, Loader2, ArrowRight, User, Palette } from "lucide-react";
import { apiClient } from "@/lib/api-client";

type PlatformRole = "buyer" | "seller";

export default function RegisterPage() {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [role, setRole] = useState<PlatformRole>("buyer");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Communicate strictly via the established backend envelope
    ...
    if (response.success) {
      // Upon 201 Created, transition the user to the login gateway
      router.push("/login");
    } else {
      setErrorMessage(response.message || "Registration failed. The identity may already exist in the ledger.");
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Tactile Role Segmented Control */}
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`flex items-center justify-center gap-2 py-3 rounded-lg border font-interface text-xs uppercase tracking-widest transition-all duration-300 ${
                role === "buyer"
                  ? "bg-white/8 border-gold-accent text-gold-accent"
                  : "bg-transparent border-white/8 text-text-muted hover:border-white/20"
              }`}
            >
              <User size={14} /> Collector
            </button>
            <button
              type="button"
        ...
        <motion.button
          ...
          type="submit"
          disabled={isLoading}
          className="w-full bg-text-primary hover:bg-gold-accent text-bg-main font-interface font-medium text-xs uppercase tracking-widest py-3 px-4 rounded-lg flex items-center justify-center gap-2 cursor-pointer transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed mt-4"
        >
          {isLoading ? (
